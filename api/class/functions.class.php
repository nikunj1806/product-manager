...
//    }
    public function send_mail($to, $cc, $bcc, $subject, $message, $alert_msg, $attachments) {
        //$this->_PHPMailer = new PHPMailer();
        $this->_mailer = new PHPMailer(true);
        try {
            $this->_mailer->AddAddress($to);
            $this->_mailer->SetFrom('krunal48@gmail.com', 'Admin');
            $this->_mailer->AddReplyTo('krunal48@gmail.com', 'Admin');
            if ($cc != "") {
                $this->_mailer->AddCC($cc);
            }
            ...
            $this->_mailer->Subject = $subject;
//            $this->_mailer->AltBody = 'To view the message, please use an HTML compatible email viewer!'; // optional - MsgHTML will create an alternate automatically
